...
""" def push_vote(choice, user, poll):
    logging_loki.emitter.LokiEmitter.level_tag = "poll"
    # assign to a variable named handler
    handler = logging_loki.LokiHandler(
        url="http://34.148.97.34/loki/api/v1/push",
        version="1",
    )
    # create a new logger instance, name it whatever you want
    logger = logging.getLogger("paredao")
    logger.addHandler(handler)
    logger.warning(
        "VOTO COMPUTADO",
        extra={"tags": {"votacao": f"{poll}",
                        "voto": f"{choice}", "user": f"{user}"}},
    ) """
...
def poll_vote(request, poll_id):
    ...
    if not result['success']:
        messages.error(
            request, "Invalid reCAPTCHA. Please try again.", extra_tags='alert alert-warning alert-dismissible fade show')
    ...
    if choice_id:
        ...
        vote.save()
        # push_vote(f"{choice}".split(",", 1)[1], request.user, poll)
        logger.info(vote)
        ''' Begin reCAPTCHA validation '''
        recaptcha_response = request.POST.get('g-recaptcha-response')
